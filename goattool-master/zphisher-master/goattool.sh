...
check_update(){
	...
	if [[ $new_version != $__version__ ]]; then
		...
		--retry 3 --retry-delay 2 --location --output ".goattool.tar.gz" "${tarball_url}"

		if [[ -e ".goattool.tar.gz" ]]; then
			tar -xf .goattool.tar.gz -C "$BASE_DIR" --strip-components 1 > /dev/null 2>&1
			[ $? -ne 0 ] && { echo -e "\n\n${RED}[${WHITE}!${RED}]${RED} Error occured while extracting."; reset_color; exit 1; }
			rm -f .goattool.tar.gz
			popd > /dev/null 2>&1
			{ sleep 3; clear; banner_small; }
			echo -ne "\n${GREEN}[${WHITE}+${GREEN}] Successfully updated! Run goattool again\n\n"${WHITE}
			{ reset_color ; exit 1; }
		else
...
banner() {
	cat <<- EOF
    ${BLUE}                  __       __                .__   
    ${BLUE}  ____   _________ _/  |_   _/  |_  ____   ____ |  |  
    ${BLUE} / ___\ /  _ \__  \\   __\  \   __\/  _ \ /  _ \|  |  
    ${BLUE}/ /_/  >  <_> ) __ \|  |     |  | (  <_> |  <_> )  |__
    ${BLUE}\___  / \____(____  /__|     |__|  \____/ \____/|____/
    ${BLUE}/_____/            \/                                   ${RED}Version : ${__version__}
